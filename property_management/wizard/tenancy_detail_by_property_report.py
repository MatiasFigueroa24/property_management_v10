...
class TenancyPropertyReport(models.TransientModel):
    ...
    def open_tenancy_by_property_gantt(self):
        ...
        for data_rec in self:
            ...
            tenancy_ids = self.env['account.analytic.account'].search(
                [('property_id', '=', property_id), ('date_start', '>=', start_date), ('date_start', '<=', end_date)])
        return {
            'view_type': 'form',
            'view_id': wiz_form_id,
            'view_mode': 'gantt',
    ...
    def print_report(self):
        ...
        return self.env['report'].get_action(self, 'property_management.report_tenancy_by_property',
                                             data=data)
